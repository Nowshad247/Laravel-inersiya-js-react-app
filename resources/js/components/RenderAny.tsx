...
const RenderAny: React.FC<RenderAnyProps> = ({ data }) => {
    // 1. Blacklist only the meta-data you don't want to see.
    // We KEEP "name" so it shows for Batch and Courses.
    ...
    // 2. Handle Arrays (The Courses list)
    ...
    // 3. Handle Objects (The Table)
    if (typeof data === 'object') {
        // If the data is wrapped in studentData, unwrap it automatically
        const sourceData = (data as any).studentData ? (data as any).studentData : data;
        const entries = Object.entries(sourceData).filter(([key]) => !blacklistedKeys.includes(key));
